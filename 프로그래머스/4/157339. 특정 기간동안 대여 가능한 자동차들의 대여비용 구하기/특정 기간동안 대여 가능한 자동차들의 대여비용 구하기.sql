SELECT DISTINCT A.CAR_ID AS CAR_ID, A.CAR_TYPE AS CAR_TYPE, ROUND(A.DAILY_FEE * 30 * (100 - C.DISCOUNT_RATE) / 100) AS FEE
FROM CAR_RENTAL_COMPANY_CAR A 
INNER JOIN CAR_RENTAL_COMPANY_RENTAL_HISTORY B ON A.CAR_ID = B.CAR_ID
INNER JOIN CAR_RENTAL_COMPANY_DISCOUNT_PLAN C ON A.CAR_TYPE = C.CAR_TYPE
WHERE A.CAR_TYPE IN ("세단", "SUV") 
AND C.DURATION_TYPE = "30일 이상"
# AND (B.START_DATE > "2022-11-30" OR B.END_DATE < "2022-11-01")
AND A.CAR_ID NOT IN ( 
        SELECT CAR_ID
        FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY
        WHERE END_DATE >= '2022-11-01' AND START_DATE <= '2022-12-01')
AND ROUND(A.DAILY_FEE * 30 * (100 - C.DISCOUNT_RATE) / 100) BETWEEN 499999 AND 2000000
ORDER BY FEE DESC, A.CAR_TYPE ASC, A.CAR_ID DESC


